...
#Path to mission.sqm required for reading
#Mission file should be in same directroy as script
scriptDirectory = os.path.dirname(os.path.realpath(__file__)) + "\\"
missionPath = scriptDirectory + "mission.sqm"
...
if os.path.isfile(missionPath):
    ...
    #Avoid issue by sanitizing the user's chosen filename
    outputFilename = raw_input("Enter a filename for the script you wish to create:")
    if (outputFilename[len(outputFilename) - 4:]) != ".sqf":
        outputFilename += ".sqf"

    #The written file should be created/overwritten in the same directory
    outputPath = scriptDirectory + outputFilename
